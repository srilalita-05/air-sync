...
def test_ventilation_coefficient_increases_with_wind():
    ...
    vc_low = calculate_ventilation_coefficient(speed_ms=1.0, pbl_height_m=500.0)
    vc_high = calculate_ventilation_coefficient(speed_ms=5.0, pbl_height_m=500.0)
...
def test_accumulation_potential_higher_for_lower_pbl():
    ...
    acc_high_pbl = calculate_accumulation_potential(speed_ms=2.0, pbl_height_m=1200.0)
    acc_low_pbl = calculate_accumulation_potential(speed_ms=2.0, pbl_height_m=200.0)
...
def test_rainfall_scavenging_reduces_pm25_concentration():
    ...
    step_no_rain = integrate_box_model_step(
        c_current=150.0, pollutant="pm25", wind_speed_ms=2.0, pbl_height_m=500.0, precipitation_mm_h=0.0
    )
    step_heavy_rain = integrate_box_model_step(
        c_current=150.0, pollutant="pm25", wind_speed_ms=2.0, pbl_height_m=500.0, precipitation_mm_h=10.0
    )
...
def test_box_model_dimensional_non_negativity():
    ...
    res = integrate_box_model_step(
        c_current=10.0, pollutant="pm25", wind_speed_ms=20.0, pbl_height_m=1000.0, precipitation_mm_h=50.0
    )
