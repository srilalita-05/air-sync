...
def calculate_plume_trajectory(
    ...
    # Base initial radius from FRP intensity (higher FRP -> larger initial thermal plume)
    initial_radius_m = 500.0 + (min(500.0, frp_mw * 5.0) if frp_mw is not None else 100.0)
    ...
    proximity_weight = math.exp(-0.5 * (min_distance_to_target_m / max(2000.0, effective_sigma))**2)
    
    # FRP intensity factor
    intensity_weight = min(1.0, (frp_mw / 200.0)) if frp_mw is not None else 0.5
    
    influence_score = round(max(0.0, min(1.0, proximity_weight * (0.4 + 0.6 * intensity_weight))), 3)
