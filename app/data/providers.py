...
class WeatherDataProvider(ABC):
    ...
    @abstractmethod
    def fetch_forecast(self, latitude: float, longitude: float, start_time_iso: str, horizon_hours: int = 72) -> List[Dict[str, Any]]:
...
class OpenMeteoWeatherProvider(WeatherDataProvider):
    ...
    def __init__(self, api_base_url: str = "https://api.open-meteo.com/v1/forecast"):
            ...
            if resp.status_code == 200:
                ...
                for i in range(min(horizon_hours + 1, len(times))):
                    output_timeline.append({
                        ...
                        "pressure_hpa": float(hourly["surface_pressure"][i]),
                        "wind_speed_ms": float(hourly["wind_speed_10m"][i]),
                        "wind_direction_deg": float(hourly["wind_direction_10m"][i]),
                        "precipitation_mm": float(hourly["precipitation"][i]),
                        "solar_radiation_wm2": float(hourly["shortwave_radiation"][i]),
                        "cloud_cover_pct": float(hourly["cloud_cover"][i]),
                        "pbl_height_m": max(100.0, float(hourly["boundary_layer_height"][i])) if hourly.get("boundary_layer_height") and hourly["boundary_layer_height"][i] is not None else 600.0,
                        "fire_influence": 0.2,
                    })
                logger.info(f"Successfully fetched live Open-Meteo weather forecast ({len(output_timeline)} hours).")
                return output_timeline
        except Exception as err:
            logger.warning(f"Live weather API fetch failed ({err}). Falling back to synthetic Delhi-NCR scenario.")

        # Graceful fallback to synthetic scenario if offline
        synth = generate_synthetic_scenario("stagnant_winter", start_time_iso, horizon_hours)
...
class ReanalysisDataProvider(ABC):
    ...
    @abstractmethod
    def fetch_historical(self, latitude: float, longitude: float, start_time_iso: str, end_time_iso: str) -> List[Dict[str, Any]]:
...
class ERA5ReanalysisProvider(ReanalysisDataProvider):
    ...
    def __init__(self, cds_url: Optional[str] = None, cds_key: Optional[str] = None):
        self.cds_url = cds_url or os.getenv("COPERNICUS_CDS_URL", "https://cds.climate.copernicus.eu/api/v2")
        self.cds_key = cds_key or os.getenv("COPERNICUS_CDS_API_KEY", "")

    def fetch_historical(self, latitude: float, longitude: float, start_time_iso: str, end_time_iso: str) -> List[Dict[str, Any]]:
        if not self.cds_key:
            logger.info("Copernicus CDS API key not configured. Using synthetic historical ERA5 reanalysis training data.")
            synth = generate_synthetic_scenario("stagnant_winter", start_time_iso, horizon_hours=168)
            return synth["timeline"]
        ...
        try:
            ...
            logger.warning("cdsapi package not installed. Run 'pip install cdsapi' for live ERA5 downloads.")
...
class OpenAQCPCBGroundProvider(GroundObservationProvider):
    ...
    def fetch_latest_observations(self, station_id: str = "DEL001") -> Dict[str, Any]:
        headers = {}
        if self.api_key:
        ...
        try:
            ...
            params = {"coordinates": "28.6139,77.2090", "radius": 25000, "limit": 1}
            resp = requests.get(self.api_url, headers=headers, params=params, timeout=4.0)
            if resp.status_code == 200:
                logger.info(f"Successfully fetched live ground station observation for {station_id}.")
        except Exception as err:
            logger.warning(f"OpenAQ ground observation fetch failed ({err}). Using standard station priors.")
...
class FireHotspotProvider(ABC):
    ...
    @abstractmethod
    def fetch_active_fires(self, bbox: Tuple[float, float, float, float], window_hours: int = 24) -> List[Dict[str, Any]]:
...
class NASAFIRMSFireProvider(FireHotspotProvider):
    ...
    def fetch_active_fires(
        ...
        if not self.map_key:
            logger.info("NASA FIRMS MAPKEY not provided. Returning synthetic fire hotspots for Punjab/Haryana stubble corridor.")
            return [
                {"latitude": 30.3165, "longitude": 75.9869, "frp_mw": 185.0, "confidence": "high", "source": "VIIRS_NPP"},
                {"latitude": 30.7333, "longitude": 76.7794, "frp_mw": 210.0, "confidence": "high", "source": "VIIRS_NPP"},
                {"latitude": 29.9695, "longitude": 76.8783, "frp_mw": 140.0, "confidence": "nominal", "source": "MODIS"},
            ]
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if len(lines) > 1:
                    ...
                    logger.info(f"Fetched {len(fires)} active fire hotspots from NASA FIRMS API.")
                    return fires
        except Exception as err:
            logger.warning(f"NASA FIRMS API fetch failed ({err}). Using hotspot baseline fallback.")
