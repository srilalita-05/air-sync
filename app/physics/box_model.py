...
def calculate_wet_scavenging_rate(
        pollutant: str,
        precipitation_mm_h: float) -> float:
...
def calculate_photochemical_o3_rate(
    ...
    # Precursor factor (NO2 proxy if available, otherwise nominal urban
    # precursor factor)
    precursor_factor = (
        no2_ugm3 /
        40.0) if (
        no2_ugm3 is not None and no2_ugm3 > 0) else 1.0
    precursor_factor = min(2.0, max(0.2, precursor_factor))

    # Production rate proxy: Peak production ~ 0.005 µg/(m³·s) under intense
    # midday sun & heat
...
def integrate_box_model_step(
    ...
    v_wet_scav = calculate_wet_scavenging_rate(
        key, precipitation_mm_h)  # Wet removal [s⁻¹]

    k_loss = v_advection + v_dry_dep + \
        v_wet_scav  # Total loss coefficient [s⁻¹]
    ...
    if key in ["pm25", "pm10"]:
        # Fire emission proxy flux addition [µg/(m²·s)]
        e_surface += fire_influence * 1.5
    elif key == "co":
        # Fire CO flux in mg/(m²·s) to match co_mgm3 unit
        e_surface += fire_influence * 0.0015

    # Volumetric surface emission source [µg/(m³·s)]
    p_emission = e_surface / h
    p_advection = v_advection * c_bg  # Inflow background source [µg/(m³·s)]

    p_chem = 0.0
    if key == "o3":
        p_chem = calculate_photochemical_o3_rate(
            temperature_c, solar_radiation_wm2, cloud_cover_pct, no2_ugm3)

    # Total volumetric production [µg/(m³·s)]
    p_total = p_emission + p_advection + p_chem
