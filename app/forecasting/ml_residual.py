...
class ResidualCorrectionModel:
    ...
    def train(self, df_training_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if df_training_data is None or len(df_training_data) < 50:
            return {
                "status": "error",
                "message": "Insufficient training records"}

        # Sort chronologically
        if "timestamp" in df_training_data.columns:
            df_training_data = df_training_data.sort_values(
        ...
        feature_cols = [
            c for c in df_training_data.columns if c not in [
                "timestamp",
                "obs_pm25",
                "obs_pm10",
                "obs_o3",
        ...
        for pol in ["pm25", "pm10", "o3"]:
            y_target = df_training_data[f"obs_{pol}"] - \
                df_training_data[f"phys_{pol}"]
            ...
            metrics_out[pol] = {
                "mae": round(
                    float(mae), 3), "rmse": round(
                    float(rmse), 3), "r2": round(
                    float(r2), 3)}
        ...
        self.is_trained = True
        joblib.dump({"metrics": metrics_out}, os.path.join(
            self.model_dir, "residual_metadata.pkl"))
    ...
    def predict_residual(
        ...
        if not self.is_trained or pol not in self.models:
            # Return 0 residual when model is not trained (unadjusted physics)
            return 0.0
