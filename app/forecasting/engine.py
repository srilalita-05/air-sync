...
def run_72h_forecast(
    ...
    start_iso = weather_timeline[0].get("timestamp", datetime.now(timezone.utc).isoformat())
    
    # Establish initial pollutant anchors at origin (t=0)
    origin_obs = {
        "pm25_ugm3": float(initial_observations.get("pm25_ugm3", weather_timeline[0].get("pm25_ugm3", 80.0)) if initial_observations else weather_timeline[0].get("pm25_ugm3", 80.0)),
        "pm10_ugm3": float(initial_observations.get("pm10_ugm3", weather_timeline[0].get("pm10_ugm3", 120.0)) if initial_observations else weather_timeline[0].get("pm10_ugm3", 120.0)),
        "o3_ugm3": float(initial_observations.get("o3_ugm3", weather_timeline[0].get("o3_ugm3", 45.0)) if initial_observations else weather_timeline[0].get("o3_ugm3", 45.0)),
        "no2_ugm3": float(initial_observations.get("no2_ugm3", weather_timeline[0].get("no2_ugm3", 35.0)) if initial_observations else weather_timeline[0].get("no2_ugm3", 35.0)),
        "so2_ugm3": float(initial_observations.get("so2_ugm3", weather_timeline[0].get("so2_ugm3", 15.0)) if initial_observations else weather_timeline[0].get("so2_ugm3", 15.0)),
        "co_mgm3": float(initial_observations.get("co_mgm3", weather_timeline[0].get("co_mgm3", 1.2)) if initial_observations else weather_timeline[0].get("co_mgm3", 1.2)),
    }
    ...
    for h, step_w in enumerate(weather_timeline):
        # Inject previous timestep output as continuity state for physics box step
        step_w_inp = dict(step_w)
        if h == 0:
            step_w_inp.update(origin_obs)
            
        # STAGE 1: Physics Baseline Step
        phys_out = estimate_pollutants(step_w_inp, previous=prev_pollutants if h > 0 else None)
        ...
        if use_ml_residual and ml_residual_engine.is_trained:
            ...
            feats = ml_residual_engine.extract_features(
                ...
                physics_baseline_step={"pm25_ugm3": phys_pm25, "pm10_ugm3": phys_pm10, "o3_ugm3": phys_o3},
        ...
        explanation = generate_explanation(step_w, diagnostics, {"pm25": final_pm25, "pm10": final_pm10, "o3": final_o3})
    ...
    episode_summary = detect_pollution_episodes(timeline_for_episode, threshold_aqi=201.0)
