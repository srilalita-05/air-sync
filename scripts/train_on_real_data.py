...
def main():
    parser = argparse.ArgumentParser(description="AirSync ML Residual Model Training Script")
    parser.add_argument("--input-csv", type=str, default=None, help="Path to input paired CSV dataset")
    ...
    if args.input_csv and os.path.exists(args.input_csv):
        print(f"Loading real historical training dataset from: {args.input_csv}")
        ...
        synth = generate_synthetic_scenario("stagnant_winter", horizon_hours=300)
        records = []
        for pt in synth["timeline"]:
            phys = estimate_pollutants(pt)
            obs_pm25 = max(0.0, phys["pm25_ugm3"] + 18.0 * np.sin(pt["hour"] / 5.0) + 4.0 * np.random.randn())
            obs_pm10 = max(0.0, phys["pm10_ugm3"] + 28.0 * np.sin(pt["hour"] / 5.0) + 6.0 * np.random.randn())
            obs_o3 = max(0.0, phys["o3_ugm3"] + 9.0 * np.cos(pt["hour"] / 6.0) + 2.0 * np.random.randn())
    ...
    for pol, metrics in result.get("metrics", {}).items():
        print(f"Pollutant: {pol.upper():<6} | MAE: {metrics['mae']:<6.2f} | RMSE: {metrics['rmse']:<6.2f} | R²: {metrics['r2']:<6.2f}")
    print("----------------------------------------------------------")
    print(f"Model artifacts successfully saved to: {ml_residual_engine.model_dir}")
